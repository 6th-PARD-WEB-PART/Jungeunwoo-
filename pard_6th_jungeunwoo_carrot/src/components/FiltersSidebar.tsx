type FiltersVariant = "sidebar" | "sheet";

const towns = ["송도동","송도1동","송도2동","연수동","동춘동","청학동"];

export default function FiltersSidebar({ variant = "sidebar" }: { variant?: FiltersVariant }) {
  const isSheet = variant === "sheet";

  return (
    <aside className={isSheet ? "" : "w-[220px]"}>
      <div className={isSheet ? "" : "bg-[#FFFFFF] rounded p-4"}>
        {/* 내부 헤더: 바텀시트에서는 숨김 (상단 바는 모달에서 제공) */}
        {!isSheet && (
